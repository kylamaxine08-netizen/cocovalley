...
// ===================================================
// 🟦 HANDLE APPROVE / CANCEL PAYMENT
// (Note: your JS currently uses handlers/payment_action.php;
// this block is here if you ever post directly to this file.)
// ===================================================
if (isset($_POST['action'])) {
  ...
  if ($payment_id > 0 && $reservation_id > 0) {
    ...
    try {
      ...
      $total_price    = floatval($resData['total_price']);
      ...
      $total_paid = floatval($paidRow['total_paid']);
      ...
      if ($action === 'approve') {
        ...
        $stmt1 = $conn->prepare("
          ...
          WHERE id = ?
        ");
        $stmt1->bind_param("sdii", $payment_status, $payment_percent, $_SESSION['user_id'], $payment_id);
        $stmt1->execute();
        $stmt1->close();
        ...
        $stmt2 = $conn->prepare("
          ...
          WHERE id = ?
        ");
        $stmt2->bind_param("si", $meName, $reservation_id);
        $stmt2->execute();
        $stmt2->close();
      ...
      if ($action === 'cancel') {
        ...
        $stmt3 = $conn->prepare("
          ...
          WHERE id = ?
        ");
        $stmt3->bind_param("ii", $_SESSION['user_id'], $payment_id);
        $stmt3->execute();
        $stmt3->close();
...
<title>GCash Billing Verification - Cocovalley Richnez Waterpark</title>
...
/* PROFILE (same style as reservation list) */
.profile {
  position: relative;
  display: flex;
  align-items: center;
...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="nav">
    ...
    <a href="admin-payment.php" class="nav-item active">
...
<main class="main">
  ...
  <div class="topbar">
    ...
    <div class="profile" id="profileBtn">
      ...
      <span><?= htmlspecialchars($meName ?? 'Admin') ?></span>
...
document.addEventListener("DOMContentLoaded", () => {
  console.log("✅ Cocovalley admin-payment loaded");
  ...
  window.openLogout = function () {
    if (confirm("Logout from Cocovalley Admin?")) {
  ...
  // Global click listener for proof-thumb images
  ...
  async function updateStatus(paymentId, reservationId, action) {
    ...
    try {
      const res = await fetch("handlers/payment_action.php", {
        method: "POST",
        headers: {
      ...
      // 🔥 AUTO-REDIRECT After Approve/Cancel
      setTimeout(() => {
        window.location.href = "admin-reservation-list.php";
      }, 500);
