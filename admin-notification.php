<?php
// =========================================================
//  Cocovalley Admin & Staff Notification System (Bubble UI)
// =========================================================
session_start();
...
$role   = $_SESSION['role'];
$meName = $_SESSION['name'] ?? "Admin/Staff";
...
/* ============================================
   FETCH ALL NOTIFICATIONS (Admin + Staff)
============================================ */
$notifications = [];
...
<head>
<meta charset="UTF-8">
<title>Notifications | Cocovalley Admin</title>
...
/* BUBBLE MIX STYLE (Option 3) */
/* Base = READ bubble (white) */
.notif-bubble{
  background:#ffffff;
...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="nav">
    ...
    <a href="admin-notification.php" class="nav-item active">
...
async function markAsRead(id) {
  try {
    ...
    const res = await fetch('admin-notification.php', {
      method: 'POST',
...
// 🔔 AUTO PLAY SOUND WHEN NEW ADMIN NOTIFICATION ARRIVES
setInterval(() => {
    fetch("admin-notification.php?json=1")
        .then(res => res.json())
        .then(data => {
            if (!data || data.length === 0) return;

            let latest = data[0]?.created_at ?? null;
            let last = localStorage.getItem("cv_last_notif_time_admin");

            // kung bago → play sound
            if (latest && latest !== last) {
                document.getElementById("notifSound").play().catch(()=>{});
                localStorage.setItem("cv_last_notif_time_admin", latest);
            }
        });
